import type { Config, GraphDoc, Lens, RenderAsset, RenderManifest, View, ViewScope } from "@contour/schema";
import { assertNever, MAX_RENDER_ASSETS } from "@contour/schema";
...
export const renderLight = (doc: GraphDoc, options: Partial<RenderOptions> = {}): RenderedSvg =>
  render(doc, { lens: options.lens ?? "architecture", theme: "light", ...options });

export const renderDark = (doc: GraphDoc, options: Partial<RenderOptions> = {}): RenderedSvg =>
  render(doc, { lens: options.lens ?? "architecture", theme: "dark", ...options });
